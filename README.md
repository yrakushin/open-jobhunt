...
- **«Все по зарплате»** — отклики без match/роли, только мин. зарплата и чёрный список
- Ответы на вопросы работодателя при отклике (из профиля / `salary_expectation`)
...
- Excel-отчёт: «Отклики» / «Требует действия» / «Пропущено» (+ колонка вопросы/ответы)
...
- macOS: иконка в Dock — `scripts/macos/install-dock-app.sh`
