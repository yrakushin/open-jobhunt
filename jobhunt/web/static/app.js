...
const SALARY_BTN_LABEL = "Все по зарплате";
...
function parseSentProgress(logText, jobMeta) {
  const text = logText || "";
  let sent = 0;
  let goal = 0;

  // Сервер держит high-water mark отдельно от урезанного лога
  if (jobMeta && typeof jobMeta.sent_count === "number") {
    sent = Math.max(sent, jobMeta.sent_count);
  }
  if (jobMeta && typeof jobMeta.sent_goal === "number" && jobMeta.sent_goal > 0) {
    goal = Math.max(goal, jobMeta.sent_goal);
  }

  // ✓ Отправлено (n/m) и mid-progress «отправлено n/m»
  const all = [...text.matchAll(/(?:✓\s*)?Отправлено\s*\(?\s*(\d+)\s*\/\s*(\d+)\s*\)?/gi)];
  for (const m of all) {
  ...
  if (dryMatches) sent = Math.max(sent, dryMatches.length);
  ...
  sent = Math.max(sent, maxSentSeen);
  goal = Math.max(goal, maxGoalSeen, getRunLimit());
  maxSentSeen = Math.max(maxSentSeen, sent);
  maxGoalSeen = Math.max(maxGoalSeen, goal);
  return { sent, goal };
}

function updateSentCounter(logText, status, jobMeta) {
  ...
  const { sent, goal } = parseSentProgress(logText, jobMeta);
  el.hidden = false;
...
function setJobBadge(status, logText, jobMeta) {
  ...
  updateSentCounter(logText ?? lastLogRaw, status, jobMeta);
}

function setRunning(running) {
  ["btn-dry", "btn-run", "btn-run-salary", "run-limit"].forEach((id) => {
...
async function startJob(url) {
  ...
  lastSentShown = -1;
  maxSentSeen = 0;
  maxGoalSeen = getRunLimit();
  setJobBadge("starting", `Цель: ${getRunLimit()} откликов`, { sent_count: 0, sent_goal: getRunLimit() });
  setLogText("Запуск…");
  setRunning(true);
  pollJob();
}

function runUrl(dryRun, salaryOnly = false) {
  const limit = getRunLimit();
  const parts = [`limit=${limit}`];
  if (dryRun) parts.push("dry_run=true");
  if (salaryOnly) parts.push("salary_only=true");
  return `/api/run?${parts.join("&")}`;
}
...
async function pollJob() {
  ...
  pollTimer = setInterval(async () => {
    ...
    const meta = { sent_count: job.sent_count, sent_goal: job.sent_goal };
    if (st === "running" || st === "starting") setJobBadge(st === "starting" ? "starting" : "running", log, meta);
    else if (st === "done") setJobBadge("done", log, meta);
    else setJobBadge("error", log, meta);
...
function resetSalaryButton() {
  const b = $("btn-run-salary");
  if (!b) return;
  b.classList.remove("confirm");
  b.textContent = SALARY_BTN_LABEL;
  salaryConfirmTimer = null;
}
let salaryConfirmTimer = null;
...
if ($("btn-run-salary")) {
  $("btn-run-salary").addEventListener("click", () => {
    const b = $("btn-run-salary");
    if (b.classList.contains("confirm")) {
      clearTimeout(salaryConfirmTimer);
      resetSalaryButton();
      startJob(runUrl(false, true));
      return;
    }
    const limit = getRunLimit();
    b.classList.add("confirm");
    b.textContent = `Без match, ${limit} шт? Ещё раз`;
    salaryConfirmTimer = setTimeout(resetSalaryButton, 4000);
  });
}
