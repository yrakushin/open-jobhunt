...
def _status() -> dict[str, Any]:
    ...
    return {
        ...
        "salary_expectation": cfg.get("profile", {}).get("salary_expectation", ""),
...
_SENT_PROGRESS_RE = re.compile(
    r"(?:✓\s*)?Отправлено\s*\(?\s*(\d+)\s*/\s*(\d+)\s*\)?",
    re.IGNORECASE,
)
_SENT_MID_RE = re.compile(r"отправлено\s+(\d+)\s*/\s*(\d+)", re.IGNORECASE)
_SENT_SUMMARY_RE = re.compile(r"Итог:\s*отправлено\s+(\d+)", re.IGNORECASE)


def _update_sent_from_line(job: dict[str, Any], line: str) -> None:
    """Держим sent_count/sent_goal отдельно от урезанного лога."""
    for rx in (_SENT_PROGRESS_RE, _SENT_MID_RE):
        m = rx.search(line)
        if m:
            sent = int(m.group(1))
            goal = int(m.group(2))
            job["sent_count"] = max(int(job.get("sent_count") or 0), sent)
            job["sent_goal"] = max(int(job.get("sent_goal") or 0), goal)
            return
    m = _SENT_SUMMARY_RE.search(line)
    if m:
        job["sent_count"] = max(int(job.get("sent_count") or 0), int(m.group(1)))


def _run_job_inprocess(job_id: str, dry_run: bool, limit: int, salary_only: bool = False) -> None:
    log_lines: list[str] = []
    ...
    def append(line: str) -> None:
        ...
        with _jobs_lock:
            if job_id in _jobs:
                ...
                _update_sent_from_line(job, line)
    ...
    _stop_flag.clear()
    mode = "только зарплата" if salary_only else ("тест" if dry_run else "отклики")
    append(f"Запуск: {mode}, лимит {limit}")
    with _jobs_lock:
        if job_id in _jobs:
            _jobs[job_id]["status"] = "running"
            _jobs[job_id]["sent_count"] = 0
            _jobs[job_id]["sent_goal"] = limit
...
def _start_job(
    ...
    with _jobs_lock:
        _jobs[job_id] = {
            ...
            "log": "Старт…",
            "sent_count": 0,
            "sent_goal": limit if args and args[0] == "run" else 0,
            "created_at": datetime.now().isoformat(),
            "updated_at": datetime.now().isoformat(),
        }
    if args and args[0] == "run":
        t = threading.Thread(
            target=_run_job_inprocess,
            args=(job_id, dry_run, limit, salary_only),
            daemon=True,
        )
    else:
...
def api_search_preferences(body: dict[str, Any]) -> dict[str, Any]:
    ...
    if min_salary:
        from jobhunt.config import format_salary_expectation

        msg += f" (в ответах: {format_salary_expectation(min_salary)})"
    if prefs.strip():
...
def api_run(
    ...
    if salary_only:
        args.append("--salary-only")
    job_id = _start_job(args, dry_run=dry_run, limit=limit, salary_only=salary_only)
