...
async def run_jobhunt(
    ...
    # UI «Мин. зарплата» — и фильтр, и ответ на вопрос работодателя о деньгах
    salary_for_qa = (
        format_salary_expectation(min_salary)
        if min_salary > 0
        else str(profile_cfg.get("salary_expectation", "") or "").strip()
    )
    ...
    try:
        ...
        if salary_only:
            log("Режим: только зарплата — без оценки match и фильтра роли")
        if age_mode != "all":
        ...
        if not salary_only and role_filter_enabled(search):
        ...
        if min_salary > 0:
            log(f"Мин. зарплата: от {min_salary // 1000}k ₽ (в ответах: {salary_for_qa})")
        ...
        with Progress(
            ...
            for vid in vacancy_ids:
                ...
                if not salary_only:
                    ...
                    log("  match: пропуск оценки (режим только зарплата)")

                try:
                    if requires_test:
                        log(f"  ⚠ в описании упомянут тест — попробую откликнуться сам")

                    if not salary_only and eval_result and eval_result.blocked:
                    ...
                    if not salary_only and score < min_match:
                    ...
                    status, apply_note, employer_qa = await session.apply_to_vacancy(
                        vid,
                        resume_hash,
                        letter,
                        salary_expectation=salary_for_qa,
                        llm=llm,
                    ...
                    if employer_qa:
                        log(f"  ответы на {len(employer_qa)} вопр. работодателя")
                        from jobhunt.core.employer_qa import format_qa_for_report

                        for line in format_qa_for_report(employer_qa).splitlines():
                            log(f"    {line}")
